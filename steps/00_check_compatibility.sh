...
if pacman -Qe gnome-shell &>>.hyprkiss.log || pacman -Qe plasma-desktop &>>.hyprkiss.log; then
...
cat /sys/firmware/efi/fw_platform_size &>>.hyprkiss.log || shitpost_error "not efi boot, get outta me swamp"

if ! command -v efibootmgr &>>.hyprkiss.log; then
    shitpost_error "efibootmgr not found, get outta me swamp"
fi

if ! command -v limine &>>.hyprkiss.log; then
    shitpost_error "limine not found, get outta me swamp"
fi

if ! command -v pipewire &>>.hyprkiss.log; then
    shitpost_error "pipewire not found, get outta me swamp"
fi

if ! command -v git &>>.hyprkiss.log; then
    shitpost_error "git not found, get outta me swamp"
fi

if ! command -v snapper &>>.hyprkiss.log; then
